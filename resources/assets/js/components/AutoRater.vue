<template>
    <div class="row">
        <div class="col-md-8">
            <auto-inputs :coverage-selection="coverage_selection" :coverage-types="coverage_types" :root-vehicles-details="vehicles_details" :root-drivers-details="drivers_details" :step="step" :latest-step="latestStep" :zip="zip"></auto-inputs>
        </div>
        <div class="col-md-4 mb-4">
            <price-panel :coverage-selection="pricing_coverage" :vehicles-length="vehicles_details.length" :root-coverage-types="coverage_types"></price-panel>

            <vehicles-box :vehicles="vehicles_details" :coverage-types="coverage_types" :coverage-selection="pricing_coverage" v-if="step > 1 || step === 'loading'"></vehicles-box>

            <div class="col-12 zip-container mt-3 text-center py-3">
                <span class="font-weight-bold">Home Zip Code</span>

                <div class="col-12 mt-2">
                    <input type="text" class="form-control" v-model="zip" name="zip" placeholder="96097" maxlength="5" :disabled="step !== 0" v-on:keypress="isNumber($event)">
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    import AutoInputs from './AutoInputs'
    ...
    export default {
        data() {
            return {
                coverage_selection: 'basic',
                pricing_coverage: 'basic',
                step: 0,
                latestStep: 0,
                zip: null,
                vehicles_details: [],
                ...
                coverage_types: {
                    basic: {
                        name: "Basic",
                        img: "/images/basic.svg",
                        injury: "$15.000 / $30.000",
                        damage: "$5.000",
                        price: 14
                    },
                    better: {
                        name: "Better",
                        img: "/images/better.svg",
        ...
        components: {
            PricePanel, AutoInputs, VehiclesBox
        },

        watch: {
            step(currStep) {
                if (currStep > this.latestStep && currStep !== 'loading') {
                    this.latestStep = currStep;
                }
            },

            coverage_selection() {
                setTimeout(() => {
                    this.pricing_coverage = this.coverage_selection
                }, 1000)
            },

            ratesData() {
                if (!this.ratesSubmitted && this.quote_id) {
                    let obj = {
                    ...
                    axios.post('/api/auto/quote/' + this.quote_id + '/update', obj).then(response => {
                        vm.ratesSubmitted = true;
                    }).catch(err => {
            ...
            ratesSubmitted(isSubmitted) {
                if (isSubmitted && this.thinkingIsDone) {
                    this.changeStep(4)
                }
            },

            thinkingIsDone(completed) {
                if (completed && this.ratesSubmitted) {
                    this.changeStep(4)
                }
            }
        },
        ...
        mounted() {
            ...
            EventBus.$on('thinking-is-done', () => {
                vm.thinkingIsDone = true;
            });

            EventBus.$on('coverage_changed', type => {
                vm.coverage_selection = type;
                vm.$nextTick(() => {
                    vm.calculatePrice();
                })
            });
            ...
            EventBus.$on('vehicles_details_changed', vehicles => {
                vm.vehicles_details = vehicles;
                vm.calculatePrice();
            });

            EventBus.$on('change_step', step => {
                this.changeStep(step);
            });

            EventBus.$on('verify-page-data-submitted', data => {
                vm.verify_page_details.address = data.address;
                vm.verify_page_details.email = data.email;
                vm.verify_page_details.phone = data.phone;
                vm.verify_page_details.is_gathered = true;

                vm.changeStep('loading');
                vm.fetchRates();
                vm.saveQuoteToDb();
            });

            $(window).on('popstate', function(event) {
                let pathName = event.currentTarget.location.pathname;

                switch (pathName) {
                    case '/auto':
                        vm.step = 0;
                        break;
                    case '/auto/vehicles-info':
                        vm.step = 1;
                        break;
                    case '/auto/drivers-info':
                        vm.step = 2;
                        break;
                }
            });
            ...
            $(window).bind('beforeunload', function(){
                if (vm.step !== 4 && vm.step !== 0) {
                    return 'Changes that you made will not be saved. Are you sure?';
                }
            });
        },
        ...
        methods: {
            changeStep(step) {
                switch(step) {
                    case 1:
                        if (!this.zip) {
                            swal("Zip Code Required!", "Please enter your zip code and try again...", "error");
                            return;
                        }
                        ...
                        if (!zips.includes(this.zip)) {
                            swal("Coming Soon!", "Your state is not yet supported. Please stay tuned...", "warning");
                            return;
                        }

                        let zipInt = parseInt(this.zip);

                        if (90001 < zipInt && zipInt < 90899) {
                            this.zipPrice = 20;
                        } else if (91001 < zipInt && zipInt < 91987) {
                            this.zipPrice = 15;
                        } else if (92003 < zipInt && zipInt < 95993) {
                            this.zipPrice = 10;
                        } else {
                            this.zipPrice = 0;
                        }
                        ...
                        window.history.pushState(null, null, '/auto/vehicles-info');
                        break;

                    case 2:
                        this.step = 2;
                        window.history.pushState(null, null, '/auto/drivers-info');
                        break;

                    case 3:
                        this.step = 3;
                        window.history.pushState(null, null, '/auto/verify');
                        break;
                    ...
                    case 4:
                        this.step = 4;
                        window.location.href = '/auto/quote/' + this.quote_id;
                        break;
                }
            },
            ...
            calculatePrice() {
                ...
                if (this.vehicles_details.length === 0) {
                    this.price = this.coverage_types[this.coverage_selection].price + this.zipPrice;
                    setTimeout(() => {
                        vm.pricing_coverage = vm.coverage_selection
                    }, 1000);

                    EventBus.$emit('price_changed', this.price);
                    return;
                }

                let count250deduct = 0;
                let count500deduct = 0;
                let count1000deduct = 0;

                this.vehicles_details.forEach(vehicle => {
                    if (vehicle.coverage === '250') {
                        count250deduct += 1;
                    } else if (vehicle.coverage === '500') {
                        count500deduct += 1;
                    } else if (vehicle.coverage === '1000') {
                        count1000deduct += 1;
                    }
                });

                switch (this.coverage_selection) {
                    case 'basic':
                        this.price = 27 * this.vehicles_details.length;

                        if (count250deduct !== 0) {
                            this.price += (27 * count250deduct + 80);
                        }
                        if (count500deduct !== 0) {
                            this.price += (27 * count500deduct + 40);
                        }
                        if (count1000deduct !== 0) {
                            this.price += (27 * count1000deduct + 20);
                        }

                        break;
                    case 'better':
                        this.price = 20 + (this.vehicles_details.length * 27);

                        if (count250deduct !== 0) {
                            this.price += (27 * count250deduct + 100);
                        }
                        if (count500deduct !== 0) {
                            this.price += (27 * count500deduct + 60);
                        }
                        if (count1000deduct !== 0) {
                            this.price += (27 * count1000deduct + 40);
                        }

                        break;
                    case 'best':
                        this.price = 30 + (this.vehicles_details.length * 27);

                        if (count250deduct !== 0) {
                            this.price += (27 * count250deduct + 110);
                        }
                        if (count500deduct !== 0) {
                            this.price += (27 * count500deduct + 70);
                        }
                        if (count1000deduct !== 0) {
                            this.price += (27 * count1000deduct + 50);
                        }
            ...
            fetchRates() {
                let obj = {
                    vehicles_details: this.vehicles_details,
                    drivers_details: this.drivers_details,
                    zip: this.zip,
                    coverage: this.coverage_selection,
                    details: {
                        address: this.verify_page_details.address,
                        email: this.verify_page_details.email,
                        phone: this.verify_page_details.phone
                    }
                };

                const vm = this;

                axios.post('/api/auto/ask-quote', obj).then(response => {
                    console.log(response.data);
                    let company = response.data.quote['Company'];
                    // let down = response.data.quote['Down Payment'];
                    let total = response.data.quote['Total Quoted Amount'];
                    ...
                    console.log(err);
                    swal("Something went wrong", "Error occurred while communicating third-party", 'error')
                })
            },
            ...
            saveQuoteToDb() {
                let obj = {
                    ...
                    zip: this.zip,
                    coverage: this.coverage_selection,
                    details: this.verify_page_details
                };

                if (!_.isEmpty(this.ratesData)) {
                    obj.rates = this.ratesData;
                    this.ratesSubmitted = true;
                }

                const vm = this;

                axios.post('/api/auto/store', obj).then(response => {
                    vm.quote_id = response.data.quote.hash_id;
                }).catch(err => {
                    swal("Something went wrong", "Error occurred while creating personalized quote. \n Please try again later.", 'error')
                })
            }
        }
    }
...
<style>
    ...
    .number-arrow {
        ...
        animation: scroll 0.7s ease-in alternate infinite;
    }
